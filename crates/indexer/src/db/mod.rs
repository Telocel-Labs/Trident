...
/// the next run.
pub async fn scan_ledger_gaps(pool: &PgPool, max_gaps: i64) -> Result<Vec<LedgerGap>, TridentError> {
...
mod tests {
    ...
    async fn enqueue_backfill_job_is_idempotent_for_pending_jobs() {
        ...
        let count: (i64,) =
            sqlx::query_as("SELECT COUNT(*) FROM backfill_jobs WHERE network = $1")
                .bind(&network)
                .fetch_one(&pool)
                .await
                .unwrap();
        assert_eq!(count.0, 1, "the same pending range must not be enqueued twice");
    ...
    async fn close_filled_backfill_jobs_closes_exact_matches_only() {
        ...
        enqueue_backfill_job(&pool, filled_gap, &network).await.unwrap();
        enqueue_backfill_job(&pool, still_open_gap, &network).await.unwrap();
