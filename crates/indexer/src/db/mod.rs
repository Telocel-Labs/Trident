...
/// Load all contract IDs and their `index_from` values from `indexed_contracts`
/// for the given network (or network-agnostic rows where `network IS NULL`).
///
/// Returns an empty map if the table has no rows — the caller treats an empty
/// map as "index all contracts" (issue #47).
///
/// The `index_from` column (BIGINT, default 0) marks the first ledger from
/// which events for that contract should be indexed. Events below this ledger
/// are silently skipped (issue #202).
pub async fn load_indexed_contracts(
    ...
    Ok(rows.into_iter().map(|(id, from)| (id, from)).collect())
}
