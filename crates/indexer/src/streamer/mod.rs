    ...
    async fn scan_and_enqueue_gaps(&mut self) {
        ...
        match db::close_filled_backfill_jobs(&self.db, &self.config.network, &gaps).await {
            Ok(closed) if closed > 0 => {
                ...
                tracing::info!(closed, "Gap scan: previously-enqueued backfill jobs confirmed filled");
            }
            Ok(_) => {}
            Err(e) => {
...
mod tests {
    ...
    async fn scan_and_enqueue_gaps_enqueues_then_closes() {
        ...
        assert_eq!(job, ((base + 1) as i64, (base + 1) as i64, "pending".to_string()));
        ...
        .await
        .unwrap();
        assert_eq!(status, "done", "the job must be closed once the gap is filled");
