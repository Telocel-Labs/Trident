...
impl Streamer {
    ...
    async fn detect_and_repair_reorg(&mut self, cursor: u64) -> Result<Option<u64>, TridentError> {
        if cursor == 0 {
        ...
        if self.last_chain_tip > 0 && self.last_chain_tip < cursor {
            ...
            return self
                .repair_reorg_from(self.last_chain_tip.saturating_add(1))
                .await;
        }
        ...
        for (position, (seq, stored_hash)) in stored.iter().enumerate() {
            ...
            let rpc_hash = match self.rpc.get_ledger(seq).await {
                ...
                Ok(None) => {
                    ...
                    tracing::warn!(
                        seq,
                        "getLedgers returned no ledger for a sequence we have stored"
                    );
...
mod tests {
    ...
    async fn reorg_detected_and_repaired_end_to_end() {
        ...
        fn commit<'a>(
            events: &'a [trident_common::SorobanEvent],
            seq: u64,
            hash: &'a str,
        ) -> db::PageCommit<'a> {
            db::PageCommit {
        ...
        assert!(
            repaired.is_some(),
            "a hash mismatch at the tip must be detected as a reorg"
        );
